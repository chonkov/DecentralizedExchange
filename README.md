...
1. Set up your enviroment variables in a '.env' file

2. Spin up the local node

```
npx hardhat node
```

3. Deploy the following scripts in that order

```
npx hardhat run scripts/deployUniswap.js
```
...
- Improve the UI by making it more stylistic and responsive
- Allow the ability for users to make 'MultiSwaps' (For now only 'WETH' to other tokens is supported)
- Fix a bug with Metamask, where 'HttpProviderErrors' are thrown
